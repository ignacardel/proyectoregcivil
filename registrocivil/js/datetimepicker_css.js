...
var DomingoColor="#C0F64F";//var DomingoColor="#C0F64F";//Background color of Domingo.
var SabadoColor="#C0F64F";//Background color of Sabado.
...
var LunesFirstDay=true;//true:Use Lunes as first day; false:Domingo as first day. [true|false]  //added in version 1.7
...
function NewCssCal(pCtrl,pFormat,pScroller,pShowTime,pTimeMode,pHideSeconds) {
	...
	if (exDateTime!="")	{ //Parse existing Date String
		...
		if ((parseInt(strDate,10)<=Cal.GetLuness()) && (parseInt(strDate,10)>=1))
			Cal.Date=strDate;
		//end parse Date
	...
	for (j=1;j<=Cal.GetLuness();j++) {
		...
		if ((j==dtToday.getDate())&&(Cal.Month==dtToday.getMonth())&&(Cal.Year==dtToday.getFullYear()))
			...
			if ((j==selDate.getDate())&&(Cal.Month==selDate.getMonth())&&(Cal.Year==selDate.getFullYear())) { //modified version 1.7
				...
				if (LunesFirstDay==true) {
					if (vDayCount%7==0)
						strCell=GenCell(j,false,DomingoColor);
					else if ((vDayCount+1)%7==0)
						strCell=GenCell(j,false,SabadoColor);
					else
		...
		if((vDayCount%7==0)&&(j<Cal.GetLuness())) {
			vCalData=vCalData+"\n</tr>";
			calHeight += 19;
		}
	}
...
function GenCell(pValue,pHighLight,pColor) { //Generate table cell with value
	...
	if (PValue!="") {
		...
		if(pColor == SabadoColor || pColor == DomingoColor || pColor == SelDateColor || pColor == TodayColor) {
...
function GetLuness() { //Get number of days in a month
...
Calendar.prototype.GetLuness=GetLuness;
